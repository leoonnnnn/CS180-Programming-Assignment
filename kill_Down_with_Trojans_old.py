...
def DP(n, H, tile_types, tile_values):
    ...
    temp = DP_helper(memo, n, tile_types, tile_values, 0, 0, 0, 0)
    res = H + temp
...
def DP_helper(memo, n, tile_types, tile_values, x, y, pTok, mTok):  #hp as param is the problem, bc having hp there is like a dif fxn call, so just do the same thing but do min hp needed and remove hp from fxn call, cuz this is acutally the 5d memo O(Hn^2) solution but with a wrong memo. The reason why max doesnt make sense, is cuz u can hit negative on the way to the end, which is the opposite direction of maximizing. And once u hit negative u end. tho, actually should still work because for the minimizing solution we still check if ans < 0 and cap at 0. Wait actually, maximizing hp gain is basically same as min hp needed.
    ...
    # if hp < 0:
    #     return -123 
    down = DP_helper(memo, n, tile_types, tile_values, x+1, y, pTok, mTok) + curval  # move down
    #print(x, y, "down")
    right = DP_helper(memo, n, tile_types, tile_values, x, y+1, pTok, mTok) + curval    # move right
    #print(x, y, "right")
    down_ptoken = -12345
    right_ptoken = -12345
    if tile_types[x][y] == 0 and pTok == 1:
        down_ptoken = DP_helper(memo, n, tile_types, tile_values, x+1, y, 0, mTok)  # use token + move down
        #print(x, y, "pdown")
        right_ptoken = DP_helper(memo, n, tile_types, tile_values, x, y+1, 0, mTok)    # use token + move right
        #print(x, y, "pright")
    down_mtoken = -12345
    right_mtoken = -12345
    if tile_types[x][y] == 1 and mTok == 1:
        down_mtoken = DP_helper(memo, n, tile_types, tile_values, x+1, y, pTok, 0) + curval * 2   # use token + move down
        #print(x, y, "mdown", down_mtoken)
        right_mtoken = DP_helper(memo, n, tile_types, tile_values, x, y+1, pTok, 0) + curval * 2    # use token + move right
        #print(x, y, "mright", right_mtoken)
    ans = max(down, right, down_ptoken, right_ptoken,  down_mtoken, right_mtoken)
    if ans < 0:    #no reviving, penalize
